...
def analyze_temperature_event(
    ...
    for m in markets:
        ...
        if is_exact_bucket(bucket_low, bucket_high):
            logger.debug("SKIP exact bucket: %s", label)
            continue

        if not forecast_hits_bucket(forecast_max, bucket_low, bucket_high):
            logger.debug(
                "SKIP forecast miss: %s forecast=%.1f not in [%s, %s]",
                label, forecast_max, bucket_low, bucket_high,
            )
            continue
