...
def place_real_bet(portfolio: RealPortfolio, opp: dict, trader: Trader) -> Optional[dict]:
    ...
    max_pct = settings.get("max_bet_pct", BET_FRACTION)
    max_usd = settings.get("max_bet_usd", 15.0)

    edge = opp.get("edge", 0)
    yes_price = opp.get("market_yes_price", 0)

    frac = _kelly_fraction(edge, yes_price, max_pct)
    if frac <= 0:
        logger.info("Kelly skip, market %s", market_id)
        return None

    bet_amount = round(usdc_balance * frac, 2)
    bet_amount = min(bet_amount, max_usd)

    if bet_amount < MIN_BET_USDC:
        logger.warning("Bet too small after Kelly: $%.2f", bet_amount)
        return None
